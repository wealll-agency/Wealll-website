const fs = require("fs");
const path =
  "d:/Kaustav/Projects/wealll_company_website/WeAlll_React/src/Components/ServiceComponent/VideoProduction/PlatformExplanatoryVideos/platform-explanatory.css";
let content = fs.readFileSync(path, "utf8");

const replacement = `
/* 4. Animated & Motion Graphic Explainers (Hover Align Storyboards) */
.storyboard-canvas {
  position: relative;
  height: 550px;
  width: 100%;
...
const startIdx = content.indexOf(
  "/* 4. Animated & Motion Graphic Explainers (Floating 3D Canvas) */",
);
const endIdx = content.indexOf(
  "/* 6. Platform Optimization & Distribution (Connected nodes) */",
);

content =
  content.substring(0, startIdx) + replacement + content.substring(endIdx);

const rplSearch = ".plat-funnel { width: 100%; }";
const rplRes = `.eq-funnel-container { flex-direction: column; align-items: stretch; height: auto; border: none; }
...
content = content.replace(rplSearch, rplRes);
